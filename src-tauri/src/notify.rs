...
//! # タップしても何も起きないことについて
//!
//! 「確認してください」と言われて押しても開かないのは不親切だが、
//! いまの構成では安全に実装できない。
//!
//! `tauri-plugin-notification` のデスクトップ実装は `notification.show()`
//! を呼んで応答を捨てており、クリックを受け取る API が無い
//! （`register_action_types` は mobile 専用）。
//!
//! 下位の `notify-rust` には `wait_for_response` があるが、**呼び出しが
//! ブロックする**。通知ごとにスレッドを起こすことになり、macOS の通知
//! コールバックはメインの run loop を必要とするため、Tauri 自身の
//! run loop と競合する。加えて `mac-notification-sys` は署名済みバンドル
//! でしか動かず、`cargo tauri dev` では確かめられない。
//!
//! 壊れ方が分かりにくいわりに得るものが小さいので、いまは入れない。
//! プラグイン側にデスクトップ用のコールバックが入ったら足す。
