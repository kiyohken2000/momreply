...
export default function App() {
  ...
  // どれか 1 つの失敗で案内まで出なくなる。
  const checkAccess = useCallback(async () => {
    try {
      setAccess(await chatDbStatus());
    } catch (e) {
      setError(String(e));
    }
  }, []);
  ...
  return (
    <div className="flex h-full flex-col text-sm">
      ...
      <main className="min-h-0 flex-1 overflow-hidden">
        {/* 権限が無いうちは他を出さない。相手も登録できず返信も来ないまま、
            何が悪いのか分からない時間が続くのが最悪の形。 */}
        {access && !access.ok ? (
          <NeedsAccess status={access} onRecheck={() => void checkAccess()} />
        ) : (
          <>
            {tab === "replies" && <Replies />}
            {tab === "targets" && <Targets />}
                  ...
                  {providers.map((p) => (
                    ...
                    >
                      <input
                        ...
                        // 生成が失敗して初めて気づくことになる。
                        disabled={
                          !p.implemented || !p.configured || !p.verified
                        }
                        onChange={async () => {
                          try {
                  ...
                  statuses.map((s) => (
                    <ApiKeyRow
                      ...
                      onChange={(next) =>
                        setStatuses((prev) =>
                          (prev ?? []).map((x) =>
                            x.provider === next.provider ? next : x,
                          ),
                        )
                      }
